...
public class Eratosthene{
  public static void main(String[]args){
    ...
    calculatePrimes(primes, 2, 30); //calculatePrimesArray takes in an empty arraylist, a minimum and a maximum
  }
  public static void calculatePrimes(ArrayList<Integer>primes, int min, int max){ //method to calculate the primes from an arbitrary array of integers
    if(max<min||min<0||max<0||min==0) //runs tests for any impossible cases and stops method calculatePrimes if cases are found
      System.exit(1);
    while(min<2)
      min++;
    ...
    //System.out.println(primes); //print statement prints out current array primes. Used to check progress
  }
